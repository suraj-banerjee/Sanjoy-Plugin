...
/**
 * Studio Dashboard Template
 * Dashboard for Design Studios
 */
...
if ($user_type !== 'studio') {
    echo '<p>This dashboard is only for Studio users.</p>';
    return;
}
...
// Get studio profile data
$studio_profile = $wpdb->get_row($wpdb->prepare(
...
if (!$studio_profile) {
    $wpdb->insert(
        ...
        ['user_id' => $user_id, 'user_type' => 'studio', 'created_at' => current_time('mysql')],
        ['%d', '%s', '%s']
    );
    $studio_profile = $wpdb->get_row($wpdb->prepare(
        "SELECT * FROM {$wpdb->prefix}skd_pl_user_profiles WHERE user_id = %d",
...
echo '<h1>Studio Dashboard Coming Soon!</h1>';
echo '<p>Welcome, ' . esc_html($user->display_name) . '</p>';
echo '<p>Studio-specific features will be available here.</p>';
